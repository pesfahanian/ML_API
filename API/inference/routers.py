...
async def upload(request: Request, _file: UploadFile = File(...)):
    ...
    [Run inference on an uploaded image.
     This file will be automatically validated to
        make sure it's a real image file.
     **Warning: The image file has to be a real image
        file with a '.jpg', '.jpeg', or '.png' suffix.**]

    Args:
        request (Request): [Request object]
        _file (UploadFile, optional): [File to be uploaded.].
            Defaults to File(...).

    Raises:
        HTTPException: [Database error.]
        HTTPException: [Model error.]
        HTTPException: [Database error.]

    Returns:
        [type]: [Result of the inference.]
    """
    ...
    query = inferences.insert().values(id=ID,
                                       timestamp=now,
                                       ip=IP,
                                       user=user,
                                       filename=file_name,
    ...
    try:
        ...
        detail = {"Description": 'Database error', "Reason": str(e)}
    ...
    if validate_uploaded_file(_temp):
        ...
        try:
            ...
            detail = {"Description": 'Model error', "Reason": str(e)}
            raise HTTPException(status_code=500, detail=detail)
        ...
        query = inferences.update().values(path=stored_path,
                                           validated=True,
                                           result={
